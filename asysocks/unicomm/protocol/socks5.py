...
class SOCKS5Request:
	...
	def from_target(target):
		...
		if target.protocol.name.startswith('SERVER') is True:
			req.CMD = SOCKS5Command.BIND

		try:
			endpoint_ip = ipaddress.ip_address(target.endpoint_ip)
			if endpoint_ip.version == 4:
				req.ATYP = SOCKS5AddressType.IP_V4
				req.DST_ADDR = endpoint_ip
			elif endpoint_ip.version == 6:
	...
	def construct(cmd, address, port):
		...
		req.RSV = 0
		try:
			endpoint_ip = ipaddress.ip_address(address)
			if endpoint_ip.version == 4:
				req.ATYP = SOCKS5AddressType.IP_V4
				req.DST_ADDR = endpoint_ip
			elif endpoint_ip.version == 6:
